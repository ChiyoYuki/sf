...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun (X : Type) (P : t_tree X -> Prop) =>
    fun (Pleaf : P t_leaf) =>
      fun Pltr =>
        fix f (tree : t_tree X) :=
          match tree with
          | t_leaf => Pleaf
          | t_branch (l, v, r) => Pltr l (f l) v r (f r)
          end.
...
Proof. 
  intros.
  induction t as [| l IHl v r IHr] using (better_t_tree_ind X).
  - simpl. reflexivity.
