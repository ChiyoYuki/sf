...
(** **** Exercise: 2 stars, advanced, optional (add_comm_informal)
    ...
    Proof: By induction on n.
      - First, suppose n = 0. We must show that
        0 + m = m + 0.
        Use theorem add_0_r, we have
        0 + m = m.
        And this follows directly from the definition of +.
      - Next, suppose n = S n', where
        n' + m = m + n'.
        We must now show that
        S n' + m = m + S n'.
        Using theorem plus_n_Sm, we have
        S n' + m = S (m + n').
        And use the definition of +, we get
        S (n' + m) = S (m + n'),
...
Proof.
  ...
  - rewrite <- mult_n_Sm, <- mult_n_Sm, <- mult_n_Sm.
    rewrite -> IHp'.
    rewrite -> add_assoc, <- (add_assoc (n * p') (m * p') n).
    rewrite -> (add_comm (m * p') n) , -> add_assoc, -> add_assoc.
    reflexivity.
Qed.
...
Proof.
  ...
  - simpl.
    rewrite mul_0_r.
    reflexivity.
  - replace (S m' * p) with (p * S m').
    + rewrite <- mult_n_Sm, <- mult_n_Sm, -> mul_comm.
      rewrite -> mult_plus_distr_r, -> mult_plus_distr_r.
      replace (p * m' * n) with (n * m' * p).
      replace (p * n) with (n * p).
      reflexivity.
      rewrite -> mul_comm. reflexivity.
      rewrite <- IHm', -> mul_comm, -> (mul_comm m' p). reflexivity.
    + rewrite -> mul_comm. reflexivity.
Qed.  
...
Lemma ntb_double_comm : forall n, nat_to_bin (double n) = double_bin (nat_to_bin n).
Proof.
  induction n as [| n' IHn'].
  - simpl. reflexivity.
